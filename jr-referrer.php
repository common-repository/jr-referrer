<?php
/*
Plugin Name: JR Referrer
Plugin URI: http://www.jakeruston.co.uk/2009/12/wordpress-plugin-jr-referrer/
...
add_action('admin_menu', 'jr_referrer_add_pages');
register_activation_hook(__FILE__,'referrer_install');

// action function for above hook
function jr_referrer_add_pages() {
    add_options_page('JR Referrer', 'JR Referrer', 'administrator', 'jr_referrer', 'jr_referrer_options_page');
}
...
if (!function_exists("jr_referrer_refresh")) {
function jr_referrer_refresh() {
update_option("jr_submitted_referrer", "0");
}
}

register_activation_hook(__FILE__,'referrer_choice');

function referrer_choice () {
if (get_option("jr_referrer_links_choice")=="") {

if (_iscurlinstalled()) {
$pname="jr_referrer";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_referrer", "1");
wp_schedule_single_event(time()+172800, 'jr_referrer_refresh'); 
} else {
...
update_option("jr_referrer_links_choice", $content);
}
}

if (get_option("jr_referrer_link_personal")=="") {
$content = curl_get_contents("http://www.jakeruston.co.uk/p_pluginslink4.php");

update_option("jr_referrer_link_personal", $content);
}
}
...
// jr_referrer_options_page() displays the page content for the Test Options submenu
function jr_referrer_options_page() {
...
<?php
    ...
    echo "<h2>" . __( 'JR Referrer Plugin Options', 'mt_trans_domain' ) . "</h2>";
$blog_url_feedback=get_bloginfo('url');
...
<p><?php _e("Number of Referrals to show", 'mt_trans_domain' ); ?> 
...
<input type="radio" name="<?php echo $data_field_name_1; ?>" value="Yes" <?php echo $change30; ?>>Yes
...
register_sidebar_widget('JR Referrer', 'show_referrer');
}
...
LIMIT 0 , $referrer_number ");
   ...
   echo $before_title.stripslashes($referrer_header).$after_title.$before_widget;
echo "<ul>";   
foreach ($rows as $rows) {
echo '<li><a href="'.$rows->url.'" rel="nofollow">'.$rows->title.'</a></li>';
}
...
$linkper=utf8_decode(get_option('jr_referrer_link_personal'));

if (get_option("jr_referrer_link_newcheck")=="") {
$pieces=explode("</a>", get_option('jr_referrer_links_choice'));
...
update_option("jr_referrer_links_choice", $new);
update_option("jr_referrer_link_newcheck", "444");
}

if (get_option("jr_submitted_referrer")=="0") {
$pname="jr_referrer";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_referrer", "1");
...
wp_schedule_single_event(time()+172800, 'jr_referrer_refresh'); 
} else if (get_option("jr_submitted_referrer")=="") {
$pname="jr_referrer";
$url=get_bloginfo('url');
$current=get_option("jr_referrer_links_choice");
...
echo '<p style="font-size:x-small">Referrer Plugin created by '.$linkper.' - '.stripslashes(get_option("jr_referrer_links_choice")).'</p>';
}
